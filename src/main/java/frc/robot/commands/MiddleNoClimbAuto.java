...
public class MiddleNoClimbAuto extends SequentialCommandGroup{
    ...
    public MiddleNoClimbAuto(CANDriveSubsystem driveSubsystem, CANFuelSubsystem fuelSubsystem, CANClimbSubystem climbSubsystem){
        addCommands(
          ...
          new AutoDrive(driveSubsystem, 0, 0.75).withTimeout(BASE_TIME * 0.5),//rotates robot 0.5 seconds
          new AutoDrive(driveSubsystem, 1, 0).withTimeout(BASE_TIME * 2)//moves robot away from tower for 2 seconds
        );
    }
}
